...
type Props = {
  subjectId: number
  location: 'meaning_section' | 'subject_page'
}
        ...
        if (synonymsValue.startsWith(' ') && selection.start < 1) {
          setTimeout(() => {
            if (synonymInputRef.current) {
              synonymInputRef.current?.setSelection(
                1,
                Math.max(selection.end, 1),
              )
            }
          }, 0)
        }
      ...
      onSave: data => {
        ...
        try {
          if (studyMaterial) {
            ...
            if (!subject) {
              console.error(
                'Subject is not defined. Tried to save user synonyms',
              )
              return
            }
      ...
      <Pressable onPress={() => synonymInputRef.current?.focus()}>
        <View style={appStyles.row}>
          ...
          <View
            style={[
              styles.inputView,
              // This will ensure the placeholder is rendered correctly when
              // there are no other predefined meanings
              {
                width: otherPredefinedMeanings.length > 0 ? undefined : '100%',
                alignItems:
                  location === 'subject_page' ? 'center' : 'flex-start',
              },
            ]}>
            <TextInput
